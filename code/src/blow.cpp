...
void runDevice(Queue<Device::Measurement>& queue, std::stop_source stop)
{
    ...
    while(not stoken.stop_requested())
    {
        ...
        std::cout << "dev token " << stoken.stop_requested() << " source " << stop.stop_requested() << " pos " << stop.stop_possible() << std::endl; 
    }
}

void saveToFile(Queue<Device::Measurement>& inputQeue,Queue<Device::Measurement>& serverQueue, std::stop_source stop)
{
    auto stoken = stop.get_token();
    std::fstream fs;
    ...
    do{
        ...
        std::cout << "wr token " << stoken.stop_requested() << " source " << stop.stop_requested() << " pos " << stop.stop_possible() << std::endl; 
    } while(not stoken.stop_requested());
    fs << "];";
    fs.close();
...
int main(int argc, char **argv)
{
    ...
    std::jthread writeThread{saveToFile, std::ref(inputQueue), std::ref(serverQueue), stop};
    
    boost::asio::io_context io_context;
    TcpServer server(io_context, [&serverQueue]()->std::optional<std::string>{
    ...
    std::jthread serverThread{[&io_context](){
        io_context.run();
    }};
